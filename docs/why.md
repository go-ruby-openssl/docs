...
`go-ruby-openssl/openssl` reimplements Ruby's `openssl` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-openssl/openssl` directly, with no Ruby runtime;
